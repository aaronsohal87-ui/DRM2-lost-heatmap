...
SHIFT_DEFINITIONS = {"NS": "00:00 – 08:59 (Night Sort — stow)", "AM": "09:00 – 13:59 (Pick, stage, dispatch)", "PM": "14:00 – 23:59 (Dispatch, RELO)"}
SHIFT_HOUR_MAP = {0:"NS",1:"NS",2:"NS",3:"NS",4:"NS",5:"NS",6:"NS",7:"NS",8:"NS",9:"AM",10:"AM",11:"AM",12:"AM",13:"AM",14:"PM",15:"PM",16:"PM",17:"PM",18:"PM",19:"PM",20:"PM",21:"PM",22:"PM",23:"PM"}
...
REQUIRED_COLS = ["Tracking ID","Sort Zone","Aisle","Cluster","Package Length","Package Width","Package Height","DSP Name","Assigned Cycle","Last Updated Time"]
CHART = (7, 2.5)
CHART_SM = (6, 2)
DSP_MAX = 20
LABEL_MAX = 25
DETAIL_COLS = ["Tracking ID","Cluster","Aisle","Sort Zone","DSP Name","Size Category","Shift","Reason"]
...
def render_shift_tab(df, total, dr, key_prefix=""):
    st.info("💡 **What's here:** See which shift loses the most parcels. Select a shift to see all reasons and every parcel.")
    cols = st.columns(3)
    ...
    if "Reason" in df.columns:
        ...
        if len(s_df) > 0:
            ...
            for reason in rc.index:
                ...
                show_cols = [c for c in ["Tracking ID","Cluster","Aisle","Sort Zone","DSP Name","Size Category"] if c in s_df.columns]
    ...
    for row in lb.itertuples():
        ...
        with st.expander(f"**{s_name}** — {count} parcels ({pct_str})"):
            if count == 0:
                ...
                cols_show = [c for c in ["Tracking ID","Dispatch Time","Cluster","Aisle","Sort Zone","DSP Name","Size Category","Reason"] if c in df.columns]
...
if mode == "Single Station":
    ...
    if uploaded_file is not None:
        ...
        if found: st.warning(f"🔒 Sensitive columns auto-removed: {', '.join(found)}")
        df = clean_data(df)
        ...
        if missing:
            ...
            if len(cl_r) > 0:
                parts = [f"#{i+1} Cluster {cl} — {n} ({round(n/len(df)*100,1)}%)" for i,(cl,n) in enumerate(cl_r.head(3).items())]
                st.info("🎯 **Cluster Priority:** " + " → ".join(parts))
            t1,t2,t3,t4,t5,t6,t7 = st.tabs(["📊 Summary","📍 Lost Locations","🚚 DSP","⏱️ Shift Rankings","📅 Day of Week","💾 Export","📋 Bridge"])
            with t1:
                st.info("💡 **What's here:** High-level breakdown by size, cluster, and reason.")
                view = st.radio("Display:", ["Chart","Table"], horizontal=True, key="sum_v")
                if view == "Chart":
                    sc = df["Size Category"].value_counts()
                    if len(sc)>0: st.pyplot(make_bar_vert(sc,"Size","Lost Parcels",f"Lost by Size ({dr})",color=["green","orange","red","darkred","grey"][:len(sc)]))
                    ...
                    if "Reason" in df.columns:
                        ...
                        if len(rc)>0: st.pyplot(make_bar_horiz(rc, f"Lost by Reason ({dr})", color="firebrick"))
                else:
            ...
            with t2:
                st.info("💡 **What's here:** Find **where** parcels are being lost. Top 10 + cluster drill-down.")
                ...
                if len(rank_data)>0:
                    if rank_view=="Chart": st.pyplot(make_bar_horiz(rank_data, f"Top 10 {rank_by}s ({dr})", color="darkred"))
                    else: st.dataframe(make_table(rank_data, rank_by, "Lost Parcels"))
                st.markdown("---")
                ...
                if clusters:
                    ...
                    if len(drill_data)>0:
                        if drill_view=="Chart": st.pyplot(make_bar_horiz(drill_data, f"Cluster {sel_cluster} — {drill_by}s"))
                        else: st.dataframe(make_table(drill_data, drill_by, "Lost Parcels"))
            ...
            with t3:
                st.info("💡 **What's here:** DSP rankings. Chart = worst-first. Table = alphabetical.")
                ...
                if dsp_view=="Chart":
                    ...
                    if len(cycle_data)>0: st.pyplot(make_bar_horiz(cycle_data, f"Lost by Cycle ({dr})", color="purple"))
                else:
                    if len(dsp_data)>0: st.dataframe(make_table(dsp_data.sort_index(),"DSP","Lost Parcels"), use_container_width=True)
                    if len(cycle_data)>0: st.dataframe(make_table(cycle_data,"Cycle","Lost Parcels"))
            ...
            with t5:
                st.info("💡 **What's here:** Day-of-week trends with values on each point.")
                ...
                if day_view=="Chart":
                    ...
                    for i,(day,val) in enumerate(day_data.items()):
                        ax.annotate(str(int(val)), xy=(i,val), xytext=(0,8), textcoords="offset points", ha="center", fontsize=8, fontweight="bold")
                    ax.set_xlabel("Day",fontsize=8); ax.set_ylabel("Lost Parcels",fontsize=8)
                    ax.set_title(f"Lost by Day of Week ({dr})",fontsize=9); ax.tick_params(labelsize=7)
                ...
                with st.expander("🔍 Parcel Details by Day"):
                    ...
                    if avail_days:
                        ...
                        out = day_df[get_detail_cols(day_df)].sort_values("DSP Name").reset_index(drop=True)
                        out.index = range(1,len(out)+1)
                        st.dataframe(out, use_container_width=True)
            with t6:
                st.info("💡 **What's here:** Download the cleaned CSV.")
                st.download_button("⬇️ Download Cleaned CSV", df.to_csv(index=False), "Lost_Parcels_Cleaned.csv", "text/csv")
            with t7:
                st.info("💡 **What's here:** Auto-generated bridge. Edit or copy prompt into Quick.")
                total = len(df)
                cl_c=df["Cluster"].dropna().value_counts(); ai_c=df["Aisle"].dropna().value_counts()
                dsp_c=df["DSP Name"].dropna().value_counts(); sz_c=df["Size Category"].value_counts()
                cy_c=df["Assigned Cycle"].dropna().value_counts(); day_c=df["Day of Week"].dropna().value_counts()
                ...
                wc=cl_c.index[0] if len(cl_c)>0 else "N/A"; wc_n=int(cl_c.values[0]) if len(cl_c)>0 else 0
                wc_p=round(wc_n/total*100,1) if total>0 else 0
                wa=ai_c.index[0] if len(ai_c)>0 else "N/A"; wa_n=int(ai_c.values[0]) if len(ai_c)>0 else 0; avg_a=ai_c.mean() if len(ai_c)>0 else 1
                wd=dsp_c.index[0] if len(dsp_c)>0 else "N/A"; wd_n=int(dsp_c.values[0]) if len(dsp_c)>0 else 0
                avg_d=dsp_c.mean() if len(dsp_c)>0 else 1; dm=round(wd_n/avg_d,1) if avg_d>0 else 1.0
                ws=sz_c.index[0] if len(sz_c)>0 else "N/A"; ws_n=int(sz_c.values[0]) if len(sz_c)>0 else 0
                wday=day_c.index[0] if len(day_c)>0 else "N/A"; wday_n=int(day_c.values[0]) if len(day_c)>0 else 0
                wsh=sh_c.index[0] if len(sh_c)>0 else "N/A"; wsh_n=int(sh_c.values[0]) if len(sh_c)>0 else 0
                wr=rs_c.index[0] if len(rs_c)>0 else "N/A"; wr_n=int(rs_c.values[0]) if len(rs_c)>0 else 0
                df["Date"]=df["Last Updated Time"].dt.strftime("%d/%m"); daily=df.groupby("Date").size()
                ...
                for cn,cv in cl_c.head(3).items():
                    ...
                    cdet+=f"  Cluster {cn}: {cv} ({round(int(cv)/total*100,1)}%) — {', '.join([f'{a} ({n})' for a,n in ta.items()])}\n"
                dlines="\n".join([f"  {d}: {n} ({round(int(n)/total*100,1)}%)" for d,n in dsp_c.head(3).items()])
                slines="\n".join([f"  {s}: {n}" for s,n in sz_c.items()])
                rlines="\n".join([f"  {r}: {n} ({round(int(n)/total*100,1)}%)" for r,n in rs_c.head(5).items()]) if len(rs_c)>0 else "  (Not in export)"
                ...
                if dm>=2: ac(f"Stand-down DSP {wd} — {dm}x average.")
                elif dm>=1.5: ac(f"Briefing DSP {wd} — {dm}x average.")
                else: ac("Station-wide refresher — losts spread evenly.")
                ...
                else: ac(f"Stow walk Cluster {wc} — {ws} parcels.")
                if avg_a>0 and wa_n>=avg_a*3: ac(f"Inspect Aisle {wa} — {round(wa_n/avg_a,1)}x avg.")
                elif avg_a>0 and wa_n>=avg_a*2: ac(f"Increase PS Aisle {wa}.")
                else: ac("Daily PS huddle with aisle rotation.")
                if len(sh_c)>1 and wsh_n>total*0.5: ac(f"5-whys {wsh} shift — {wsh_n} losts.")
                if len(rs_c)>0 and wr_n>total*0.3: ac(f"Deep-dive '{wr}' — {wr_n} parcels ({round(wr_n/total*100,1)}%).")
                relo=sum(int(cy_c.get(c,0)) for c in cy_c.index if "RELO" in str(c).upper())
                if relo>total*0.15: ac(f"RELO review — {relo} parcels.")
                if len(daily)>1 and wday_n>total*0.3: ac(f"Staffing review {wday} — {wday_n} losts.")
                ...
                prompt=f"Write a professional Lost Parcels bridge for DRM2. RC1-RC5 + AC1-AC5+.\nData ({dr}): Total={total}, Cluster={wc} ({wc_n},{wc_p}%), Aisle={wa} ({wa_n}), DSP={wd} ({wd_n},{dm}x), Size={ws} ({ws_n}), Day={wday} ({wday_n}), Shift={wsh} ({wsh_n}), Reason={wr} ({wr_n})\nDaily: {dl}\nShifts: {sl}\nClusters:\n{cdet}DSPs:\n{dlines}\nReasons:\n{rlines}\nGenerate specific actions."
                st.code(prompt, language="text")
                st.caption("📋 Copy → Quick → paste → AI-enhanced bridge")
    ...
    if len(uploaded) >= 2:
        ...
        for i, file in uploaded.items():
            tmp=clean_data(pd.read_csv(file)); name=get_station_name(tmp, file.name)
            stations[name]=tmp; names.append(name)
        ...
        for n in names:
            ...
            if len(cr)>0:
                parts=[f"#{i+1} Cluster {c} — {v} ({round(v/len(stations[n])*100,1)}%)" for i,(c,v) in enumerate(cr.head(3).items())]
                st.info(f"🎯 **{n} priority:** {' → '.join(parts)}")
        t1,t2,t3,t4,t5,t6,t7 = st.tabs(["📊 Summary","📍 Lost Locations","🚚 DSP","⏱️ Shift Rankings","📅 Day of Week","💾 Export","📋 Bridge"])
        with t1:
            st.info("💡 **What's here:** Compare totals, sizes, and loss reasons across stations.")
            ...
            if view=="Chart":
                ...
                for b in bars: ax.text(b.get_x()+b.get_width()/2,b.get_height()+0.3,str(int(b.get_height())),ha="center",fontsize=8,fontweight="bold")
                ax.set_ylabel("Lost Parcels",fontsize=8); ax.set_title("Total Lost by Station",fontsize=9)
                ax.tick_params(labelsize=7); plt.tight_layout(); st.pyplot(fig)
                fig2,ax2=plt.subplots(figsize=(8,2.5)); x=range(len(SIZE_ORDER)); w=0.8/len(names)
                for i,n in enumerate(names):
                ...
                ax2.set_ylabel("Lost",fontsize=8); ax2.set_title("Size Comparison",fontsize=9)
                ax2.legend(fontsize=7); ax2.tick_params(labelsize=7); plt.tight_layout(); st.pyplot(fig2)
                if any("Reason" in stations[n].columns for n in names):
                    st.markdown("---"); st.subheader("❓ Loss Reasons by Station")
                    for i,n in enumerate(names):
                        if "Reason" in stations[n].columns:
                            rc=stations[n]["Reason"].dropna().value_counts().head(5)
                            if len(rc)>0: st.pyplot(make_bar_horiz(rc,f"{n} — Top Reasons",color=STATION_COLORS[i],figsize_width=6))
            else:
        ...
        with t2:
            ...
            for i,n in enumerate(names):
                ...
                if len(d)>0:
                    if rank_view=="Chart": st.pyplot(make_bar_horiz(d,f"{n} — Top 10 {rank_by}s",color=STATION_COLORS[i],figsize_width=6))
                    else: st.subheader(n); st.dataframe(make_table(d,rank_by,"Lost Parcels"))
            st.markdown("---"); st.subheader("🔍 Station Drill-Down")
            for i,n in enumerate(names):
                with st.expander(f"📍 {n} — Cluster Detail",expanded=False):
                    cls=sorted(stations[n]["Cluster"].dropna().unique())
                    if cls:
                        sel=st.selectbox(f"Cluster ({n}):",cls,key=f"mc_cl_{i}")
                        flt=stations[n][stations[n]["Cluster"]==sel]
                        st.write(f"**{len(flt)} parcels**")
                        ai=flt["Aisle"].dropna().value_counts().head(10)
                        if len(ai)>0: st.pyplot(make_bar_horiz(ai,f"{n} — Cluster {sel} Aisles",color=STATION_COLORS[i],figsize_width=6))
        with t3:
            st.info("💡 **What's here:** DSP comparison. Chart=worst-first. Table=A–Z.")
            dsp_view=st.radio("Display:",["Chart","Table"],horizontal=True,key="mc_dsp_v")
            if dsp_view=="Chart":
                for i,n in enumerate(names):
                    dsp=stations[n]["DSP Name"].dropna().value_counts().head(10)
                    if len(dsp)>0: st.pyplot(make_bar_horiz(dsp,f"{n} — Worst DSPs",color=STATION_COLORS[i],max_label=DSP_MAX))
            else:
                for i,n in enumerate(names):
                    with st.expander(f"🚚 {n} — DSP List (A–Z)",expanded=False):
                        dsp=stations[n]["DSP Name"].dropna().value_counts().sort_index()
                        if len(dsp)>0: st.dataframe(make_table(dsp,"DSP","Lost Parcels"),use_container_width=True)
        with t4:
            st.info("💡 **What's here:** Compare shifts across stations.")
            fig_sh,ax_sh=plt.subplots(figsize=CHART); x=range(len(SHIFT_ORDER)); w=0.8/len(names)
            for i,n in enumerate(names):
                sd=stations[n][stations[n]["Shift"]!="Unknown"]["Shift"].value_counts()
                cts=[sd.get(s,0) for s in SHIFT_ORDER]
                bars=ax_sh.bar([xi+(i-len(names)/2+0.5)*w for xi in x],cts,w,label=n,color=STATION_COLORS[i])
                for b in bars:
                    h=b.get_height()
                    if h>0: ax_sh.text(b.get_x()+b.get_width()/2,h+0.2,str(int(h)),ha="center",fontsize=6)
            ax_sh.set_xticks(x); ax_sh.set_xticklabels(SHIFT_ORDER,fontsize=8)
            ax_sh.set_ylabel("Lost Parcels",fontsize=8); ax_sh.set_title("Shift Comparison",fontsize=9)
            ax_sh.tick_params(labelsize=7); ax_sh.legend(fontsize=7); plt.tight_layout(); st.pyplot(fig_sh)
            st.markdown("---")
            for i,n in enumerate(names):
        ...
        with t5:
            st.info("💡 **What's here:** Day-of-week comparison with values on each point.")
            fig,ax=plt.subplots(figsize=CHART)
            for i,n in enumerate(names):
                if "Day of Week" in stations[n].columns:
                    ...
                    for j,(day,val) in enumerate(dd.items()):
                        ax.annotate(str(int(val)),xy=(j,val),xytext=(0,8),textcoords="offset points",ha="center",fontsize=7,color=STATION_COLORS[i])
            ax.set_ylabel("Lost Parcels",fontsize=8); ax.set_title("Lost by Day of Week",fontsize=9)
            ax.tick_params(labelsize=7); ax.legend(fontsize=7); plt.xticks(rotation=0); plt.tight_layout(); st.pyplot(fig)
        with t6:
            st.info("💡 **What's here:** Download per station or combined.")
            for n in names: st.download_button(f"⬇️ Download {n}",stations[n].to_csv(index=False),f"Lost_{n}.csv","text/csv",key=f"dl_{n}")
            combined=pd.concat([stations[n].assign(Station_Name=n) for n in names],ignore_index=True)
            st.download_button("⬇️ Download All (Combined)",combined.to_csv(index=False),"Lost_Combined.csv","text/csv",key="dl_all")
        with t7:
            st.info("💡 **What's here:** Station comparison + Quick prompt.")
            comp=[]
            for n in names:
                sdf=stations[n]; sk=sdf[sdf["Shift"]!="Unknown"]["Shift"]
                comp.append({"Station":n,"Total Lost":len(sdf),"Worst Cluster":safe_top(sdf["Cluster"]),"Worst Aisle":safe_top(sdf["Aisle"]),"Worst DSP":safe_top(sdf["DSP Name"]),"Worst Shift":safe_top(sk) if len(sk)>0 else "N/A","Top Reason":safe_top(sdf["Reason"]) if "Reason" in sdf.columns else "N/A"})
            st.dataframe(pd.DataFrame(comp,index=range(1,len(comp)+1)),use_container_width=True)
            best=min(names,key=lambda n:len(stations[n])); worst=max(names,key=lambda n:len(stations[n]))
            gap=len(stations[worst])-len(stations[best])
            st.write(f"**Best:** {best} ({len(stations[best])}) | **Worst:** {worst} ({len(stations[worst])}) | **Gap:** {gap}")
            summ="\n".join([f"- {n}: {len(stations[n])} losts, cluster={safe_top(stations[n]['Cluster'])}, DSP={safe_top(stations[n]['DSP Name'])}, reason={safe_top(stations[n]['Reason']) if 'Reason' in stations[n].columns else 'N/A'}" for n in names])
            st.code(f"Compare stations — what can {worst} learn from {best}?\n{summ}\nBest={best}, Worst={worst}, Gap={gap}\nAnalyse loss reasons and recommend improvements.",language="text")
            st.caption("📋 Copy → Quick → paste → AI comparison")
    elif len(uploaded)==1: st.warning("⚠️ Upload at least 2 station files.")
    else: st.info("👆 Upload your CSV files above to get started.")
