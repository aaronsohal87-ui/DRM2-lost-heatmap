...
def render_shift_tab(df, total, dr, key_prefix=""):
    ...
    st.info("💡 **What's here:** See which shift loses the most parcels. "
            "Select a shift below to see all the reasons parcels were lost and the full parcel list.")
    ...
    # *** REASONS BY SHIFT — user selects a shift, sees all reasons + parcels ***
    if "Reason" in df.columns:
        st.subheader("❓ Top Reasons by Shift")
        st.caption("Select a shift to see all the reasons parcels were lost, with Tracking IDs.")
        shift_pick = st.selectbox("Select Shift:", SHIFT_ORDER, key=f"{key_prefix}reason_shift_pick")
        s_df = df[(df["Shift"] == shift_pick) & (df["Reason"].notna())]
        if len(s_df) > 0:
            st.markdown(f"**{shift_pick}** — {len(s_df)} parcels with a reason recorded")
            # Count table
            ...
            # Full parcel list grouped by reason
            st.caption("Full parcel list grouped by reason:")
            for reason in rc.index:
                reason_df = s_df[s_df["Reason"] == reason]
                show_cols = [c for c in ["Tracking ID", "Cluster", "Aisle",
                             "Sort Zone", "DSP Name", "Size Category"] if c in reason_df.columns]
            ...
            st.info(f"No parcels with a recorded reason on {shift_pick} shift.")
        st.markdown("---")

    # Per-shift parcel detail (expandable)
...
if mode == "Single Station":
    ...
    if uploaded_file is not None:
        ...
        if missing:
            ...
            if len(cl_r) > 0:
                parts = [f"#{i+1} Cluster {cl} — {n} parcels ({round(n/len(df)*100,1)}%)"
            ...
            with t1:
                ...
                if view == "Chart":
                    ...
                    if len(sc) > 0:
                        st.pyplot(make_bar_vert(sc, "Size", "Lost Parcels", f"Lost by Size ({dr})",
                    ...
                    st.dataframe(pivot)
                    if "Reason" in df.columns:
                        st.dataframe(make_table(df["Reason"].dropna().value_counts(), "Reason", "Lost Parcels"),
                                     use_container_width=True)

            with t2:
                st.info("💡 **What's here:** Find **where** parcels are being lost. Top 10 rankings + cluster drill-down.")
                ...
                if clusters:
                    ...
                    if len(drill_data) > 0:
                        if drill_view == "Chart":
                            st.pyplot(make_bar_horiz(drill_data, f"Cluster {sel_cluster} — {drill_by}s"))
                        else:
            ...
            with t3:
                ...
                if dsp_view == "Chart":
                    ...
                    if len(dsp_data) > 0:
                        st.dataframe(make_table(dsp_data.sort_index(), "DSP", "Lost Parcels"), use_container_width=True)
                    if len(cycle_data) > 0:
            ...
            with t5:
                ...
                if day_view == "Chart":
                    ...
                    ax.tick_params(labelsize=7); plt.xticks(rotation=0); plt.tight_layout()
                    st.pyplot(fig)
                else:
            ...
            with t7:
                st.info("💡 **What's here:** Auto-generated bridge. Edit or copy the prompt into Quick.")
                total = len(df)
    ...
    if len(uploaded) >= 2:
        ...
        with t1:
            ...
            if view == "Chart":
                ...
                for b in bars:
                    ax.text(b.get_x()+b.get_width()/2, b.get_height()+0.3, str(int(b.get_height())), ha="center", fontsize=8, fontweight="bold")
            ...
            for i,n in enumerate(names):
                ...
                if len(d) > 0:
                    if rank_view == "Chart":
                        ...
                        st.subheader(n); st.dataframe(make_table(d, rank_by, "Lost Parcels"))
        ...
        with t7:
            st.info("💡 **What's here:** Station comparison table + Quick prompt.")
            comp = []
            for n in names:
                ...
                comp.append({"Station":n, "Total Lost":len(sdf), "Worst Cluster":safe_top(sdf["Cluster"]),
                    ...
                    "Top Reason":safe_top(sdf["Reason"]) if "Reason" in sdf.columns else "N/A"})
