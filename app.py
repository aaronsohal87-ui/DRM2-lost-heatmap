...
REQUIRED_SCC_COLS = ["Tracking ID", "Sort Zone", "Aisle", "Cluster",
    ...
    "Assigned Cycle", "Last Updated Time"]
...
def merge_data(pm_df, scc_df):
    """Merge PM (master) + SCC (detail). PM = source of truth for total.
    
    DAY OF WEEK: Uses PM 'event_datetime' — the date/time each parcel was
    MARKED AS LOST. This gives a realistic spread across the week because
    parcels are flagged as lost on different days as they are identified.
    """
    scc_clean = clean_scc(scc_df.copy())
    pm_keep = ["tracking_id", "bucket", "sub_bucket", "previous_event_datetime",
               "previous_reason", "previous_reason_3", "event_datetime"]
    ...
    # event_datetime = when parcel was MARKED AS LOST (used for Day of Week)
    if "event_datetime" in pm_cols.columns:
        pm_cols["Marked Lost DT"] = pd.to_datetime(
            pm_cols["event_datetime"], dayfirst=True, errors="coerce")
    merged = pm_cols.merge(scc_clean, on="Tracking ID", how="left")
    merged["Sub Bucket"] = merged["sub_bucket"]
    merged["Bucket"] = merged.get("bucket")
    # Day of Week = day parcel was MARKED AS LOST (from PM event_datetime)
...
def make_table(series, c1, c2):
    t = series.reset_index(); t.columns = [c1, c2]
    t.index = range(1, len(t) + 1); return t
...
def make_bar_vert(data, xl, yl, title, color="steelblue", figsize=CHART):
    ...
    plt.xticks(rotation=0); plt.tight_layout(); return fig
...
def make_bar_shift(data, title):
    ...
    ax.set_xlabel("Shift", fontsize=8); ax.set_ylabel("Lost", fontsize=8)
    ax.set_title(title, fontsize=9); ax.tick_params(labelsize=7)
    plt.xticks(rotation=0); plt.tight_layout(); return fig
...
def make_pie_otr_utr(df, total, title):
    ...
    ax.set_title(title, fontsize=9); plt.tight_layout(); return fig

def render_locations_tab(df, total, dr, key_prefix=""):
    st.info("💡 **OTR** = DSP + reason. **UTR** = loss reasons. **All** = cluster/aisle.")
    verify_totals(df, total, "Locations")
    loc_filter = st.radio("Show:", ["All Parcels", "OTR Only (Lost On Road)", "UTR Reprocess Only"],
                          horizontal=True, key=f"{key_prefix}lf")
    if loc_filter == "OTR Only (Lost On Road)":
        ...
        with st.expander("🚚 DSP → Reason Drill-Down"):
            ...
            if dsps:
                ...
                if len(dsp_reasons) > 0:
                    st.dataframe(make_table(dsp_reasons,"Reason Given","Count"), use_container_width=True)
        ...
        with st.expander("❓ All OTR Reasons (summary)"):
...
with st.expander("📖 How to get your data"):
    st.markdown("""
**1.** PerfectMile → Concessions Control Tower → L&U → Lost → Export CSV
**2.** SCC → paste Tracking IDs → View Options → Select All → Export CSV
**3.** Upload both below. Perfect Mile = source of truth for total count.

**Day of Week** = date each parcel was **marked as lost** (PM `event_datetime`).
    """)
    ...
    if pm_file and scc_file:
        ...
        with t1:
            ...
            with st.expander("📏 Parcel Size Breakdown"):
                ...
                if len(sc) > 0:
                    ...
                    if vm == "Chart":
                        st.pyplot(make_bar_vert(sc,"Size","Lost",f"Parcel Size ({dr})",
        ...
        with t3:
            ...
            with st.expander("🚚 DSP Breakdown"):
                ...
                if len(dsp_data) > 0:
                    vm = st.radio("Display:", ["Chart","Table"], horizontal=True, key="dsp_v")
                    if vm == "Chart": st.pyplot(make_bar_horiz(dsp_data, f"By DSP ({dr})", color="orange", max_label=DSP_MAX))
                    else: st.dataframe(make_table(dsp_data.sort_index(),"DSP","Lost"), use_container_width=True)
            ...
            if "Day of Week" in df.columns:
                ...
                with st.expander("📅 Day of Week"):
                    ...
                    if vm == "Chart":
                        ...
                        ax.set_title(f"Lost by Day Marked ({dr})",fontsize=9)
                    ...
                    has_day = df["Day of Week"].notna().sum()
                    st.caption(f"ℹ️ {has_day}/{total} parcels have date data.")
        ...
        with t6:
            ...
            exc = ["Prev Event DT","previous_event_datetime","bucket","sub_bucket",
                   "previous_reason","previous_reason_3","event_datetime","Marked Lost DT"]
        ...
        with t3:
            for i,n in enumerate(names):
                with st.expander(f"🚚 {n} — DSPs"):
                    ...
                    if len(dsp) > 0:
                        vm = st.radio("Display:", ["Chart","Table"], horizontal=True, key=f"mc_d_{n}")
                        if vm == "Chart": st.pyplot(make_bar_horiz(dsp, f"{n} DSPs", color=STATION_COLORS[i], max_label=DSP_MAX))
                        else: st.dataframe(make_table(dsp,"DSP","Lost"), use_container_width=True)
        with t4:
