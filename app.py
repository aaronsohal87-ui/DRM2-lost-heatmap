    ...
    if missing:
        ...
        with tab1:
            ...
            if overview_display == "Table":
                ...
                ax_ov.set_title(f"Lost Parcels by Size ({start_date} - {end_date})") # chart title with date range
                plt.xticks(rotation=0, ha="center") # keeps labels horizontal
                plt.tight_layout() # stops labels getting cut off
                st.pyplot(fig_ov) # displays chart in app
        ...
        with tab2:
            ...
            size_df = filtered_df[filtered_df["Size Category"] == selected_size] # filters to only that size within selected cluster
        ...
        with tab3:
            ...
            dsp_data = df["DSP Name"].dropna().value_counts() # counts losts per DSP across whole station
        ...
        with tab4:
            ...
            time_display = st.radio("Display as:", ["Chart", "Table"], horizontal=True, key="time_display") # toggle between chart and table
