...
DSP_MAX = 20
LABEL_MAX = 25
DETAIL_COLS = ["Tracking ID", "Cluster", "Aisle", "Sort Zone", "DSP Name", "Size Category", "Shift", "Sub Bucket"]
...
def merge_data(pm_df, scc_df):
    ...
    pm_keep = ["tracking_id", "bucket", "sub_bucket", "previous_event_datetime",
               "previous_reason", "previous_reason_3", "dsp"]
    pm_cols = pm_df[[c for c in pm_keep if c in pm_df.columns]].copy()
    pm_cols = pm_cols.rename(columns={"tracking_id": "Tracking ID", "dsp": "PM DSP"})
    ...
    if "previous_reason" in merged.columns:
        merged["Loss Reason"] = merged["previous_reason"].replace("NOREASON", pd.NA)
    else:
        merged["Loss Reason"] = pd.NA
    if "previous_reason_3" in merged.columns:
        merged["UTR Reason"] = merged["previous_reason_3"].replace("NOREASON", pd.NA)
    else:
        merged["UTR Reason"] = pd.NA
...
def make_bar_horiz(data, title, color="steelblue", figsize_width=7, max_label=LABEL_MAX):
    ...
    ax.set_xlabel("Lost Parcels", fontsize=8); ax.set_title(title, fontsize=9)
...
def make_bar_vert(data, xl, yl, title, color="steelblue", figsize=CHART):
    ...
    plt.xticks(rotation=0, ha="center"); plt.tight_layout()
    return fig
...
def make_bar_shift(data, title):
    ...
    ax.set_xlabel("Shift", fontsize=8); ax.set_ylabel("Lost Parcels", fontsize=8)
    ax.set_title(title, fontsize=9); ax.tick_params(labelsize=7)
...
def make_pie_otr_utr(df, total, title):
    ...
    ax.pie(sizes, labels=labels, colors=colors, explode=explode, autopct="%1.1f%%", startangle=90, textprops={"fontsize": 8})
    ax.set_title(title, fontsize=9); plt.tight_layout()
    return fig

def make_table(series, c1, c2):
    t = series.reset_index(); t.columns = [c1, c2]; t.index = range(1, len(t)+1)
    return t

def verify_totals(df, total, label=""):
    if len(df) != total:
        st.error(f"⚠️ MISMATCH {label}: Expected {total}, got {len(df)}.")
        return False
    return True


# ─── SUGGESTED OPPORTUNITIES (all in expanders) ──────────────────────────────
...
def render_opportunities_tab(df, total, dr, key_prefix=""):
    st.info("💡 Each parcel assigned to the responsible shift via sub-bucket + last scan time. "
            "Expand sections below to explore.")
...
""")
    ...
    with st.expander("🏆 Shift Responsibility Leaderboard"):
        ...
        if len(shift_counts) > 0:
            st.pyplot(make_bar_shift(shift_counts, f"Lost by Shift ({dr})"))
    ...
    with st.expander("🔍 Shift Drill-Down"):
        st.caption("Select a shift to see its sub-bucket breakdown.")
        ...
        selected = st.selectbox("Select Shift:", shift_options, key=f"{key_prefix}opp_sel")
        selected_shift = selected.split(" — ")[0]
        s_df = df[df["Shift"] == selected_shift]
        count = len(s_df)
        if count > 0:
            pct = round(count / total * 100, 1)
            st.markdown(f"**{selected_shift}** — **{count} parcels** ({pct}%)")
            ...
            show_cols = [c for c in ["Tracking ID","Sub Bucket","Cluster","Aisle","DSP Name","Size Category"] if c in df.columns]
...
# ─── LOST LOCATIONS (OTR=DSP+reason, UTR=reason, All=cluster/aisle) ──────────

def render_locations_tab(df, total, dr, key_prefix=""):
    st.info("💡 Find where parcels are lost. OTR shows DSP + reason. UTR shows loss reasons.")
    verify_totals(df, total, "Locations")
    filter_options = ["All Parcels", "OTR Only (Lost On Road)", "UTR Reprocess Only"]
    loc_filter = st.radio("Show:", filter_options, horizontal=True, key=f"{key_prefix}loc_f")

    if loc_filter == "OTR Only (Lost On Road)":
        view_df = df[df["Sub Bucket"].str.contains("Lost On Road", na=False)].copy()
        st.write(f"**Showing: {len(view_df)} OTR parcels**")
        ...
        with st.expander("🚚 OTR by DSP"):
            ...
            if len(dsp_data) > 0:
                st.pyplot(make_bar_horiz(dsp_data, f"OTR Lost by DSP ({dr})", color="firebrick", max_label=DSP_MAX))
                st.dataframe(make_table(dsp_data, "DSP", "Lost Parcels"), use_container_width=True)
            else:
                st.info("No DSP data for OTR parcels.")
        with st.expander("❓ OTR Loss Reasons"):
            if "Loss Reason" in view_df.columns:
                ...
                if len(reason_data) > 0:
                    ...
                    st.info("No reason data for OTR parcels.")
            else:
                st.info("No reason column available.")
        with st.expander("📦 All OTR parcels"):
            show_cols = [c for c in ["Tracking ID","Sub Bucket","DSP Name","Loss Reason","Cluster","Aisle"] if c in view_df.columns]
            out = view_df[show_cols].sort_values("DSP Name").reset_index(drop=True)
            out.index = range(1, len(out)+1)
            st.dataframe(out, use_container_width=True)

    elif loc_filter == "UTR Reprocess Only":
        view_df = df[df["Sub Bucket"] == "Lost At Station - UTR Reprocess"].copy()
        st.write(f"**Showing: {len(view_df)} UTR parcels**")
        if len(view_df) == 0: st.warning("No UTR parcels."); return
        with st.expander("❓ UTR Loss Reasons"):
            if "UTR Reason" in view_df.columns:
                reason_data = view_df["UTR Reason"].dropna().value_counts()
                if len(reason_data) > 0:
                    st.dataframe(make_table(reason_data, "Reason", "Count"), use_container_width=True)
                else:
                    st.info("No specific reasons recorded (all NOREASON).")
            else:
                st.info("No reason column available.")
        with st.expander("📍 UTR by Cluster/Aisle"):
        ...
        with st.expander("📦 All UTR parcels"):
            ...
            out = view_df[show_cols].sort_values("Cluster").reset_index(drop=True)
            out.index = range(1, len(out)+1)
            st.dataframe(out, use_container_width=True)

    else:
        view_df = df.copy()
        st.write(f"**Showing: {len(view_df)} parcels (all)**")
        ...
        with st.expander("🔍 Cluster Drill-Down"):
            ...
            if clusters:
                ...
                drill_by = st.selectbox("View by:", ["Aisle", "Sort Zone"], key=f"{key_prefix}dr")
                drill_data = filt[drill_by].dropna().value_counts()
                if len(drill_data) > 0:
                    st.pyplot(make_bar_horiz(drill_data, f"Cluster {sel} — {drill_by}s", color="steelblue"))
...
with st.expander("📖 How to get your data"):
    st.markdown("**1.** PerfectMile → L&U → Lost → Export CSV\n"
                "**2.** SCC → paste Tracking IDs → Select All → Export CSV\n"
                "**3.** Upload both below. PM = source of truth for total.")
...
if mode == "Single Station":
    ...
    if pm_file and scc_file:
        ...
        with t1:
            ...
            with st.expander("📏 Size Breakdown"):
                ...
                if len(sc)>0: st.pyplot(make_bar_vert(sc,"Size","Lost",f"By Size ({dr})",
                    color=["green","orange","red","darkred","grey"][:len(sc)]))
            with st.expander("📍 Cluster Breakdown"):
                cc = df["Cluster"].dropna().value_counts()
                if len(cc)>0: st.pyplot(make_bar_horiz(cc, f"By Cluster ({dr})"))
            with st.expander("🏷️ Sub Bucket Breakdown"):
                sb = df["Sub Bucket"].value_counts()
                if len(sb)>0: st.pyplot(make_bar_horiz(sb, f"By Sub Bucket ({dr})", color="teal"))
        ...
        with t3:
            ...
            with st.expander("🚚 DSP Breakdown"):
                dsp_data = df["DSP Name"].dropna().value_counts()
                if len(dsp_data)>0: st.pyplot(make_bar_horiz(dsp_data, f"By DSP ({dr})", color="orange", max_label=DSP_MAX))
            with st.expander("📦 All parcels by DSP"):
                ...
                out.index = range(1, len(out)+1)
                st.dataframe(out, use_container_width=True)
        with t4: render_opportunities_tab(df, total, dr, key_prefix="s_")
        with t5:
            ...
            if "Day of Week" in df.columns:
                ...
                with st.expander("📅 Day of Week Chart"):
                    fig, ax = plt.subplots(figsize=CHART)
                    ax.plot(day_data.index, day_data.values, marker="o", color="green", linewidth=2)
                    ...
                    ax.set_title(f"By Day ({dr})",fontsize=9); ax.tick_params(labelsize=7)
                    plt.xticks(rotation=0); plt.tight_layout(); st.pyplot(fig)
        with t6:
            verify_totals(df, total, "Export")
            exc = ["Prev Event DT","previous_event_datetime","bucket","sub_bucket","previous_reason","previous_reason_3"]
    ...
    if len(uploaded) >= 2:
        ...
        for i,(pf,sf) in uploaded.items():
            pt, st2 = pd.read_csv(pf), pd.read_csv(sf)
            m = merge_data(pt, st2)
        ...
        with t1:
            ...
            ax.set_ylabel("Lost",fontsize=8); ax.set_title("Total by Station",fontsize=9); ax.tick_params(labelsize=7)
        ...
        with t5:
            ...
            ax.set_ylabel("Lost",fontsize=8); ax.set_title("By Day",fontsize=9)
            ax.tick_params(labelsize=7); ax.legend(fontsize=7); plt.xticks(rotation=0); plt.tight_layout(); st.pyplot(fig)
        with t6:
            for n in names:
                ec = [c for c in stations[n].columns if c not in ["Prev Event DT","previous_event_datetime","bucket","sub_bucket","previous_reason","previous_reason_3"]]
